import sys; sys.path.append("scheduler")
...
def create_problem_instance(num_jobs, cluster_spec,
    ...
    oracle_throughputs = utils.read_all_throughputs_json_v2("scheduler/simulation_throughputs.json")
    rng = random.Random()
    rng.seed(seed)
    jobs = {}
    throughputs = {}
...
def sweep(policy_names_and_num_sub_clusters,
    ...
    for num_jobs in all_num_jobs:
        ...
        for i in range(num_trials):
            ...
            allocations = []
            for j, (policy_name, num_sub_clusters) in enumerate(policy_names_and_num_sub_clusters):
                num_threads = 16 // num_sub_clusters
                policy = utils.get_policy(policy_name, solver='MOSEK',
