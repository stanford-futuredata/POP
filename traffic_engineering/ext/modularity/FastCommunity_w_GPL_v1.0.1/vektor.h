...
public:
	...
	sktuple	*heap_ptr;			// pointer to element's location in vektor max-heap
...
public:
	...
	sktuple	returnMaxKey();							// returns the maximum key in the tree
	sktuple	returnMaxStored();							// returns a sktuple of the maximum (key, .stored)
...
sktuple vektor::returnMaxStored() { return heap->returnMaximum(); }

sktuple vektor::returnMaxKey() {
	sktuple themax;
	element *current;
	current = root;
...
void vektor::deleteItem(int killKey) {
	...
	if (z != NULL) {
		sktuple newmax    = heap->returnMaximum();		// get old maximum in O(1)
...
void vektor::printTree() {
	sktuple max = heap->returnMaximum();
