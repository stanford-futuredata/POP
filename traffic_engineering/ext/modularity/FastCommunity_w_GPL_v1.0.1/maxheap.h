...
public:
	...
	sktuple	*insertItem(const sktuple newData);			// heap-inserts newData, returns the address of it
	sktuple	popMaximum();							// removes and returns heap max, reheapifies
	sktuple	returnMaximum();						// returns the heap max; no change to heap
	void		printHeap();							// displays contents of the heap
	void		printHeapTop10();						// displays top 10 entries in the heap
	void		updateItem(sktuple *address, sktuple newData);   // updates the value of the tuple at address
	void		updateItem(sktuple *address, double newStored);// update only the stored value of tuple at address
	void		deleteItem(sktuple *address);				// remove an item from the heap
	int		returnArraysize();						// 
	int		returnHeaplimit();						// 
...
// Pop Maximum ------------------------------------------------------------------------
sktuple maxheap::popMaximum() {				// O(log k) time
	sktuple temp = returnMaximum();
	deleteItem(A[1].d);
	return temp;
}

// Return Maximum --------------------------------------------------------------------
sktuple maxheap::returnMaximum() {			// O(1) time
	sktuple temp;
	temp.m = A[1].d->m;					// 
	temp.i = A[1].d->i;					// 
	temp.j = A[1].d->j;					// 
...
sktuple* maxheap::insertItem(const sktuple newData) {			// O(log k) time
	int index;
	sktuple *pointer;
...
void maxheap::updateItem(sktuple *address, sktuple newData) {   // O(log k) time
...
void maxheap::updateItem(sktuple *address, double newStored) {	// O(log k) time
	double oldm    = address->m;
...
void maxheap::deleteItem(sktuple *address) {
	sktuple *swap;
